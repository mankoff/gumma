...
def get_GUM_usage(user, passwd):
    LOCAL = True   # debug with local gumma.html saved from account screen
    LOCAL = False
    if not LOCAL:
...
class gumma(rumps.App):
    ...
    def t(self,sender):
        with app.open('user', 'r') as f:
            user = f.read()
        if user == '':
            w = rumps.Window(message='Go US Mobile login name/email',
                             title='Login',
                             default_text="your@email.com")
            r = w.run()
            user = r.text
            w = rumps.Window(message='Go US Mobile login password',
                             title='Login',
